...
#
#  Prerequisites:
#    1. Android Studio installed (bundles JDK 17)
#       https://developer.android.com/studio
#    2. Copy your signing.keystore into this folder
...
if (-not (Test-Path $AS_JDK)) {
    Write-Error @"
Android Studio JDK not found at:
  $AS_JDK
...
if (Test-Path $KEY_ENV) {
    ...
    Write-Warning ".keystore-env not found in $SCRIPT_DIR"
    Write-Warning "Copy .keystore-env.template → .keystore-env and fill in passwords."
    Write-Warning "Continuing with existing environment variables (if any)."
}

# Set KEYSTORE_PATH relative to the android/app/ directory
if (-not $env:KEYSTORE_PATH) {
    $env:KEYSTORE_PATH = "signing.keystore"
}
# Resolve the keystore: check script dir first, then android/app/
...
if (Test-Path $KS_IN_SCRIPT) {
    ...
    Write-Error "signing.keystore not found. Copy your keystore to: $SCRIPT_DIR\signing.keystore"
    exit 1
}
...
Write-Host ""
Write-Host "[3/4] Running Gradle bundleRelease (first run takes ~10 min to download Gradle)..." -ForegroundColor Cyan
Push-Location $ANDROID_DIR
.\gradlew.bat bundleRelease
if ($LASTEXITCODE -ne 0) { Write-Error "Gradle build failed"; exit 1 }
Pop-Location

# ── Step 6: Copy output .aab to this folder ────────────────────────────────
Write-Host ""
