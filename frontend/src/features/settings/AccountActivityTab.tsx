      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-5">
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 rounded-xl flex items-center justify-center" style={{ background: 'var(--primary-highlighted-color)' }}>
      ...
      {!isFetching && logs.length > 0 && (
        <div className="border border-gray-100 rounded-xl overflow-hidden">
          <div className="flex items-center gap-3 px-3 py-2 bg-gray-50 border-b border-gray-100">
            <button onClick={toggleSelectAll} className="flex-shrink-0 text-gray-400">
              {allSelected ? <CheckSquare size={15} style={{ color: 'var(--primary-color)' }} /> : <Square size={15} />}
            </button>
          ...
          {logs.map((log: any) => (
            ...
            >
              ...
              <div className="w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0" style={{ background: 'var(--primary-highlighted-color)' }}>
    ...
    <div className="layer-card p-6">
      ...
      <div className="flex gap-1 mb-5 p-1 bg-gray-100 rounded-xl w-fit">
        <button
          ...
          style={activeRole === 'HR' ? { color: 'var(--primary-color)' } : undefined}
          className={cn(
            'px-4 py-1.5 rounded-lg text-sm font-medium transition-all',
            activeRole === 'HR' ? 'bg-white shadow-sm font-semibold' : 'text-gray-500 hover:text-gray-700'
          )}
        >
          HR Activity
        </button>
        <button
          ...
          style={activeRole === 'EMPLOYEE' ? { color: 'var(--primary-color)' } : undefined}
          className={cn(
            'px-4 py-1.5 rounded-lg text-sm font-medium transition-all',
            activeRole === 'EMPLOYEE' ? 'bg-white shadow-sm font-semibold' : 'text-gray-500 hover:text-gray-700'
          )}
        >
          Employee Activity
        </button>
