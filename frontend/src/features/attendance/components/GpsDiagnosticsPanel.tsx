...
const ALERT_RULES: Record<string, (v: string) => boolean> = {
  ...
  lastWatchdogResult:                  (v) => v.startsWith('no_credentials') || v.startsWith('failed') || v.startsWith('blocked_permission'),
  lastRestartResult:                   (v) => v.startsWith('failed') || v.startsWith('missing_credentials') || v.startsWith('blocked_permission') || v === 'deferred_to_user_unlocked',
  ...
  // Permission gate
  fgsStartBlocked:                     (v) => v === 'true',
  fineLocationGranted:                 (v) => v === 'false',
  backgroundLocationGranted:           (v) => v === 'false',
  foregroundServiceLocationPermission: (v) => v === 'false',
  locationEnabled:                     (v) => v === 'false',
  lastFgsStartResult:                  (v) => v.startsWith('failed'),
  permissionGateResult:                (v) => v.startsWith('blocked'),
};
...
const OK_RULES: Record<string, (v: string) => boolean> = {
  ...
  // Permission gate
  fgsStartBlocked:                     (v) => v === 'false',
  fineLocationGranted:                 (v) => v === 'true',
  backgroundLocationGranted:           (v) => v === 'true',
  foregroundServiceLocationPermission: (v) => v === 'true',
  locationEnabled:                     (v) => v === 'true',
  lastFgsStartResult:                  (v) => v === 'success',
  permissionGateResult:                (v) => v.startsWith('allowed'),
};
...
const GROUPS: Array<{ label: string; keys: string[] }> = [
  ...
  {
    label: 'FGS Permission Gate',
    keys: [
      'permissionGateResult', 'fgsStartBlocked', 'fgsStartBlockedReason',
      'lastFgsStartAttemptAt', 'lastFgsStartResult', 'lastFgsStartException',
      'fineLocationGranted', 'coarseLocationGranted', 'backgroundLocationGranted',
      'notificationPermissionGranted', 'foregroundServiceLocationPermission',
      'locationEnabled',
    ],
  },
  {
