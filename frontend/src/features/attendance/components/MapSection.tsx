...
// ─────────────────────────────────────────────────────────────
// GPS Trail Section — for FIELD employees
// ─────────────────────────────────────────────────────────────

interface GpsTrailSectionProps {
  gpsTrail: any[];
  gpsVisits?: any[];
  selectedDate: string;
}

export function GpsTrailSection({ gpsTrail, gpsVisits = [], selectedDate }: GpsTrailSectionProps) {
  ...
  if (gpsTrail.length === 0) return null;
  ...
  for (let i = 1; i < positions.length; i++) {
    ...
    const acc1 = (gpsTrail[i - 1]?.accuracy ?? 0) / 1000;
    const acc2 = (gpsTrail[i]?.accuracy ?? 0) / 1000;
    if (segKm > Math.max(acc1, acc2)) totalKm += segKm;
  }

  const firstTs = gpsTrail[0]?.timestamp ?? gpsTrail[0]?.time ?? gpsTrail[0]?.recordedAt;
  const lastTs = gpsTrail[gpsTrail.length - 1]?.timestamp ?? gpsTrail[gpsTrail.length - 1]?.time ?? gpsTrail[gpsTrail.length - 1]?.recordedAt;
  ...
  return (
    <div className="layer-card overflow-hidden">
      ...
      <div className="px-4 pt-3 pb-2">
        ...
        <div className="grid grid-cols-3 gap-2">
          <div className="bg-indigo-50 rounded-lg px-2 py-1.5 text-center">
            <p className="text-sm font-bold font-mono text-indigo-600" data-mono>{gpsTrail.length}</p>
            <p className="text-[9px] text-gray-500">Points</p>
          </div>
          <div className="bg-emerald-50 rounded-lg px-2 py-1.5 text-center">
            <p className="text-sm font-bold font-mono text-emerald-600" data-mono>{totalKm.toFixed(2)} km</p>
            <p className="text-[9px] text-gray-500">Distance</p>
          </div>
          <div className="bg-amber-50 rounded-lg px-2 py-1.5 text-center">
            <p className="text-sm font-bold font-mono text-amber-600" data-mono>{durationMs > 0 ? fmtDuration(durationMs) : '--'}</p>
      ...
      <div style={{ height: 300 }} className="relative">
        <MapContainer center={positions[0]} zoom={13} style={{ height: '100%', width: '100%' }}>
          ...
          {gpsTrail.map((p: any, i: number) => {
            ...
            return (
              <CircleMarker key={i} center={[lat, lng]} radius={isFirst || isLast ? 8 : 5}
          ...
          {gpsVisits.map((v: any, i: number) => {
            ...
            return (
              <CircleMarker key={`v-${i}`} center={[Number(v.lat), Number(v.lng)]} radius={14}
        ...
        <button onClick={() => setTrailFullscreen(true)}
      ...
      {trailFullscreen && (
        <div className="fixed inset-0 z-[1000] flex flex-col bg-gray-900">
          <div className="flex items-center justify-between px-4 py-2 bg-gray-900 border-b border-gray-700 flex-shrink-0">
            ...
            <button onClick={() => setTrailFullscreen(false)}
            ...
            return (
              <div key={i} className="flex items-center gap-2 text-[10px] py-0.5">
                <div className={cn('w-1.5 h-1.5 rounded-full flex-shrink-0',
                  isFirst ? 'bg-emerald-500' : isLast ? 'bg-red-500' : 'bg-indigo-400')} />
                <span className="text-gray-400 font-mono w-[72px] flex-shrink-0 tabular-nums" data-mono>{fmtTime(ts)}</span>
                <span className="text-gray-600 flex-1 truncate">
                  {lat.toFixed(4)}, {lng.toFixed(4)}
                  {speedKmh != null && <span className="text-gray-400 ml-1">· {speedKmh} km/h</span>}
                </span>
                {p.accuracy != null && <span className="text-gray-400 flex-shrink-0">±{Math.round(p.accuracy)}m</span>}
              </div>
            );
          })}
        </div>
      ...
      {/* Visit stops with editable labels */}
      {gpsVisits.length > 0 && (
                ...
                <div key={i} className="text-[10px] bg-orange-50 rounded px-2 py-1.5 border border-orange-100">
                  {isEditing ? (
                    <div className="flex items-center gap-1.5">
                      ...
                      />
                      <button onClick={saveLabel} disabled={isSavingLabel} className="text-green-600 hover:text-green-700 p-0.5"><Check size={12} /></button>
                      <button onClick={() => setLabelEdit({ visitId: null, index: null, value: '' })} className="text-gray-400 hover:text-gray-600 p-0.5"><X size={12} /></button>
                    </div>
                  ) : (
                    <div className="flex items-center gap-2">
                      <div className="w-1.5 h-1.5 rounded-full bg-orange-400 flex-shrink-0" />
                      <span className="text-orange-700 font-medium flex-shrink-0">{displayName || `Stop ${i + 1}`}</span>
                      {!displayName && v.startTime && <span className="text-gray-500">{fmtTime(v.startTime)}</span>}
                      {!displayName && v.endTime && <><span className="text-gray-400">→</span><span className="text-gray-500">{fmtTime(v.endTime)}</span></>}
                      {dwellMs > 0 && <span className="text-orange-600 ml-auto font-medium">{fmtDuration(dwellMs)}</span>}
                      {v.id && (
                        <button onClick={() => setLabelEdit({ visitId: v.id, index: i, value: v.customName || '' })}
                          className="text-orange-300 hover:text-orange-600 p-0.5 ml-1 flex-shrink-0" title="Name this location">
                          <Edit2 size={10} />
                        </button>
                      )}
                    </div>
...
// ─────────────────────────────────────────────────────────────
// Check-in Location Section — for all shift types
// ─────────────────────────────────────────────────────────────
...
export function CheckInSection({ checkInLoc, geofenceCoords, geofence, geofenceViolation }: CheckInSectionProps) {
  const [officeFullscreen, setOfficeFullscreen] = useState(false);

  if (!checkInLoc?.lat) return null;
  ...
  return (
    <div className="layer-card overflow-hidden">
      ...
      <div style={{ height: 200 }} className="relative">
        ...
        >
          <TileLayer url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" attribution="&copy; OSM" />
          <CircleMarker center={[checkInLoc.lat, checkInLoc.lng]} radius={8}
        ...
        <button onClick={() => setOfficeFullscreen(true)}
          className="absolute top-3 right-3 z-[500] bg-white/90 backdrop-blur-sm border border-gray-200 rounded-lg p-1.5 shadow-md hover:bg-white transition-colors"
          title="View fullscreen">
          <Maximize2 size={15} className="text-gray-600" />
        </button>
      ...
      {officeFullscreen && (
        <div className="fixed inset-0 z-[1000] flex flex-col bg-gray-900">
          <div className="flex items-center justify-between px-4 py-2 bg-gray-900 border-b border-gray-700 flex-shrink-0">
            ...
            <button onClick={() => setOfficeFullscreen(false)}
              className="flex items-center gap-1.5 px-3 py-1.5 bg-gray-700 hover:bg-gray-600 text-white rounded-lg text-xs transition-colors">
              <Minimize2 size={14} /> Exit Fullscreen
            </button>
          </div>
          <div className="flex-1">
            <MapContainer center={[checkInLoc.lat, checkInLoc.lng]} zoom={16} style={{ height: '100%', width: '100%' }}>
              <TileLayer url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" attribution="&copy; OSM" />
              <CircleMarker center={[checkInLoc.lat, checkInLoc.lng]} radius={10}
                pathOptions={{ fillColor: '#4f46e5', color: '#fff', weight: 2, fillOpacity: 0.9 }}>
                <Popup>
...
// ─────────────────────────────────────────────────────────────
// Inline Check-in Modal — opened from Daily Records row picker
// Renders a centered modal with just the check-in map + fullscreen
// ─────────────────────────────────────────────────────────────
...
export function CheckInModal({ checkInLoc, geofenceCoords, geofence, geofenceViolation, date, onClose }: CheckInModalProps) {
      ...
      <div className="fixed inset-0 z-[1100] flex flex-col bg-gray-900">
        <div className="flex items-center justify-between px-4 py-2 bg-gray-900 border-b border-gray-700 flex-shrink-0">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={onClose} className="p-1.5 bg-gray-700 hover:bg-gray-600 text-white rounded-lg transition-colors">
              <X size={14} />
            </button>
          </div>
        </div>
    ...
    <div className="fixed inset-0 z-[600] flex items-center justify-center bg-black/60 p-4" onMouseDown={e => e.target === e.currentTarget && onClose()}>
      <div className="bg-white rounded-2xl shadow-2xl overflow-hidden w-full max-w-lg">
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100">
          ...
          <div className="flex items-center gap-1.5">
            {geofenceViolation ? (
              <span className="text-[9px] px-1.5 py-0.5 bg-red-50 text-red-600 rounded-full border border-red-200 font-medium">Outside Geofence</span>
            ) : geofenceCoords ? (
              <span className="text-[9px] px-1.5 py-0.5 bg-emerald-50 text-emerald-600 rounded-full border border-emerald-200 font-medium">Inside Zone</span>
            ) : null}
            <button onClick={() => setFullscreen(true)}
              className="p-1.5 hover:bg-gray-100 rounded-lg transition-colors" title="Fullscreen">
        ...
        <div style={{ height: 360 }}>
          <MapContainer center={[checkInLoc.lat, checkInLoc.lng]} zoom={15} style={{ height: '100%', width: '100%' }}>
            <TileLayer url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" attribution="&copy; OSM" />
            <CircleMarker center={[checkInLoc.lat, checkInLoc.lng]} radius={9}
              pathOptions={{ fillColor: '#4f46e5', color: '#fff', weight: 2, fillOpacity: 0.9 }}>
              <Popup>
                <div style={{ fontSize: 11, lineHeight: 1.6 }}>
                  <strong>📍 Check-in Point</strong>
...
// ─────────────────────────────────────────────────────────────
// Default export (kept for backward-compat if anything lazy-imports it)
// ─────────────────────────────────────────────────────────────
  ...
  if (shiftType === 'FIELD' || workMode === 'FIELD_SALES') {
    return (
      <>
        ...
        <CheckInSection checkInLoc={checkInLoc} geofenceCoords={geofenceCoords} geofence={geofence} geofenceViolation={geofenceViolation} />
      </>
    );
  }
  return <CheckInSection checkInLoc={checkInLoc} geofenceCoords={geofenceCoords} geofence={geofence} geofenceViolation={geofenceViolation} />;
}
