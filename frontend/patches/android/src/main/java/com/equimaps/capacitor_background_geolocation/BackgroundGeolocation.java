...
public class BackgroundGeolocation extends Plugin {
    ...
    public void addWatcher(final PluginCall call) {
        ...
        // Pass backend credentials so the service can post directly when app is killed
        String credBackendUrl = call.getString("backendUrl");
        String credAuthToken = call.getString("authToken");
        if (credBackendUrl != null && credAuthToken != null && !credBackendUrl.isEmpty()) {
            service.setCredentials(credBackendUrl, credAuthToken);
        }
    ...
    public void load() {
        ...
        // Start the service explicitly so it becomes a started service (not bound-only).
        // This, combined with android:stopWithTask="false" and START_STICKY, means the
        // service survives when the user force-closes the app from Recent Apps.
    ...
    protected void handleOnDestroy() {
        // Do NOT stop the service on activity destroy — the service must outlive
        // the activity so GPS tracking continues when the app is closed/force-killed.
        // The service only stops when the employee checks out (removeWatcher removes
        // all watchers, which calls stopSelf() internally).
